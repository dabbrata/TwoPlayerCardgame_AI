...
# Cards for players
ph = [1,1,1,1,1,1,1,1,0]
pc = [1,1,1,1,1,1,1,1,0]
...
def minimax(ph, pc, rnd, max_mode, alpha, beta, depth):
  
    if (rnd == mx_rnd) or game_over(ph,pc,rnd) or depth == 6:
        ...
        for i in range(cards):
            if ph[i] == 0:
                ...
                for j in range(cards):
                    ...
                    elif (((j+1) % cards) == i) or (((j+2) % cards) == i) or (((j+3) % cards) == i):

                        pc[cards] += 1
                        pc[j] = 0
                            
                        val = minimax(ph, pc, rnd+1, False, alpha, beta, depth + 1)
        ...
        for i in range(cards):
            if ph[i] == 0:
                ...
                for j in range(cards):
                    ...
                    elif (((i+1) % cards) == j) or (((i+2) % cards) == j) or (((i+3) % cards) == j):

                        ph[cards] += 1
                        ph[i] = 0
                        
                        val = minimax(ph, pc, rnd+1, True, alpha, beta, depth + 1)
        ...
        if ph[i] == 0:  
            ...
            for j in range(cards):
                if pc[j] == 0:
                    ...
                    val = minimax(ph, pc, rnd+1, False, -10, 10, 0)
...
while not game_over(ph,pc,rnd):
    ...
    if (h_move == ai_move) or (((h_move + 4) % cards) == ai_move):
        print("Its a draw. Retake")
        continue
    
    elif (((h_move + 1) % cards) == ai_move) or (((h_move + 2) % cards) == ai_move) or (((h_move + 3) % cards) == ai_move):
        ph[cards] += 1
        ph[h_move] = 0
